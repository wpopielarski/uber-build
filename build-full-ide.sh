...
: ${SBT_IDE_VERSION:=}            # The version of the sbt artifacts published by dbuild
...
then
    if [ -n "$SBT_VERSION" ]
    then
      SBT_MAVEN_OPTIONS="-Dsbt.version=${SBT_VERSION}"
    fi
    if [ -n "$SBT_IDE_VERSION" ]
    then
      SBT_MAVEN_OPTIONS="${SBT_MAVEN_OPTIONS} -Dsbt.ide.version=${SBT_IDE_VERSION}"
    fi
else
    if [[ -z "$SBT_VERSION" ]]
    then
        abort "SBT_VERSION cannot be empty"
    fi

    if [[ -z "$SBT_IDE_VERSION" ]]
    then
        abort "SBT_IDE_VERSION cannot be empty"
    fi
    SBT_MAVEN_OPTIONS="-Dsbt.version=${SBT_VERSION} -Dsbt.ide.version=${SBT_IDE_VERSION}"
fi
...
echo -e "----------------------------------------------\n"
echo -e "Sbt version       : ${SBT_VERSION}"
